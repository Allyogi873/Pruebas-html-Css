    ...
    var productos = [
        {
          id: 1,
          img:
            "http://libertadproof.com/wp-content/uploads/2016/02/87952_Obv.jpg",
          name: "Libertad 5oz",
          price: 299.0,
          desc:
            "Libertad 5oz BU 1998 Contains 1 Libertad 5oz BU brilliant uncirculated .999 fine Silver. In capsule The same coin as you see in this picture. We only Ship to the US, and is FREE Shipping Shipping time 5-7 business days via UPS express with tracking and insurance. Payments only via Paypal.",
          stock: 4
        },
        {
          id: 2,
    ...
    for (var i = 0; i < productos.length; i++) {
      if (productos[i].stock > 0) {
        ...
        contenido +=
          ...
          " USD</span></h3>";
        contenido +=
          '			<h3>Tenemos: <span class="stock">' +
          productos[i].stock +
          "</span></h3>";
        contenido += "		</span>";
        contenido +=
          '		<a class="large-12 columns btn submit ladda-button prod-' +
          productos[i].id +
          '" data-style="slide-right" onclick="app.addtoCart(' +
          productos[i].id +
          ');">Añadir a la canasta</a>';
    ...
    if (
      ...
      _.forEach(cart.items, function (n, key) {
        ...
        items +=
          ...
          "," +
          n.available +
          ')"><i class="icon ion-minus-circled"></i></button> <button onclick="app.deleteProd(' +
          n.id +
          ')" ><i class="icon ion-close-circled"></i></button><div class="clearfix"></div></h3>';
        items += "</li>";
      });
  ...
  app.updatePayForm = function () {
    ...
    var cart =
      JSON.parse(localStorage.getItem("cart")) != null
        ...
        : { items: [] };
    var statics =
        '<form action="https://www.paypal.com/cgi-bin/webscr" method="post"><input type="hidden" name="cmd" value="_cart"><input type="hidden" name="upload" value="1"><input type="hidden" name="currency_code" value="USD" /><input type="hidden" name="business" value="' +
        business_paypal +
        '">',
      dinamic = "",
      wrapper = $("#submitForm");
    ...
    if (undefined != cart && null != cart && cart != "") {
      ...
      statics +=
        dinamic +
        '<button type="submit" class="pay">Pagar &nbsp;<i class="ion-chevron-right"></i></button></form>';
